...
import com.longbei.im_push_service_sdk.im.lpresenter.dataCenter.user.UserDispatcher;
...
public class MessageDispatcher implements MessageCenter {
    ...
    @Override
    public void test_dispatch(String obj) {
        System.out.println("处理推送来的消息,丢到单线程池中进行保存");
        // 丢到单线程池中
        executor.execute(new test_MessageCardHandler(obj));
    }
        ...
        public void run() {
            ...
            for (MessageCard card : cards) {
                ...
                if (message != null) {
                    ...
                    sender.setId(card.getSenderId());
                    User receiver = null;
                    receiver = new User();
                    receiver.setId(card.getReceiverId());
                    Group group = null;
    ...
    private class test_MessageCardHandler implements Runnable {
        private String messages = "";
        test_MessageCardHandler(String messages) {
        ...
        @Override
        public void run() {
            System.out.println("处理推送来的消息,单线程池执行保存");
            DbHelper.test_save(Message.class, messages);
        }
    }
}
