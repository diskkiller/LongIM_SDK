...
public class DbHelper {
    ...
    public static <Model extends BaseModel> void test_save(final Class<Model> tClass,String obj) {
        System.out.println("处理推送来的消息,保存成功，唤起通知");
        instance.test_notifySave(tClass, obj);
    }
    ...
    private final <Model extends BaseModel> void test_notifySave(final Class<Model> tClass,
                                                            final String obj) {
        System.out.println("处理推送来的消息,唤起通知，查找监听器");
        // 找监听器
        final Set<ChangedListener> listeners = getListeners(tClass);
        if (listeners != null && listeners.size() > 0) {
            // 通用的通知
            for (ChangedListener<Model> listener : listeners) {
                System.out.println("处理推送来的消息,找到监听器进行分发");
                listener.test_onDataSave(obj);
            }
        }
    }
     ...
     */
    public void updateSession(Message... messages) {
        // 标示一个Session的唯一性
        ...
        definition.beginTransactionAsync(new ITransaction() {
            ...
            public void execute(DatabaseWrapper databaseWrapper) {
                ...
                for (Session.Identify identify : identifies) {
                    ...
                    if(!session.getId().equals(Common.curSessionId)){
                        System.out.println("session :"+session.getUnReadCount());
    ...
    public interface ChangedListener<Data extends BaseModel> {
        ...
        default void test_onDataSave(String obj){
